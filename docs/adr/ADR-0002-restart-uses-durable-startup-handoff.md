...
Restart continuation is delivered as a durable startup handoff message under `.codexteamup/exchange/startup/system/restart/` in the target checkout. The target CTU runtime reads that message through the startup-sweep loop and performs the internal dispatch itself.
